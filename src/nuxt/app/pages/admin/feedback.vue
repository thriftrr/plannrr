...
useHead({ title: 'Feedback inbox · Plannrr' })
</script>
  ...
  <main class="page">
    <div class="top">
      ...
      <div class="filters" role="tablist" aria-label="Filter">
        <button v-for="f in (['open', 'resolved', 'all'] as const)" :key="f" class="chip" :class="{ on: filter === f }" role="tab" :aria-selected="filter === f" @click="filter = f">
          {{ f }}
        </button>
      </div>
      <button class="y-btn-outline reload" :disabled="pending" @click="reload()">↻ Refresh</button>
    </div>
    <p class="lede">
      Everything sent through the floating Feedback button. Bodies are Markdown, rendered here with raw HTML disabled.
    </p>

    <p v-if="error" class="err">Could not load feedback: {{ (error.data as { statusMessage?: string } | undefined)?.statusMessage ?? error.message }}</p>
    <p v-if="actionError" class="err">{{ actionError }}</p>

    <p v-if="!pending && !items.length" class="empty">
      {{ filter === 'open' ? 'Inbox zero. Nothing open right now.' : 'Nothing here.' }}
    </p>

    <article v-for="item in items" :key="item.id" class="card y-card" :class="{ resolved: item.resolvedAt }">
      <header class="card-head">
        ...
        <div class="meta">
          ...
          <span v-if="item.page" class="sep">·</span>
          <code v-if="item.page" :title="item.page">{{ item.page }}</code>
        </div>
      </header>

      <!-- eslint-disable-next-line vue/no-v-html -- markdown-it output with html:false; user tags are escaped, never rendered -->
      <div class="body md" v-html="render(item.body)" />

      <footer class="card-foot">
        <span v-if="item.userAgent" class="ua" :title="item.userAgent">{{ item.userAgent }}</span>
        <div class="acts">
          <template v-if="confirmDelete === item.id">
            <span class="sure">Delete for good?</span>
            <button class="y-btn-danger small" :disabled="busy === item.id" @click="remove(item)">Yes, delete</button>
            <button class="y-btn-secondary small" :disabled="busy === item.id" @click="confirmDelete = null">Keep</button>
          </template>
          <template v-else>
            <button class="y-btn-secondary small" :disabled="busy === item.id" @click="confirmDelete = item.id">Delete</button>
            <button class="small" :class="item.resolvedAt ? 'y-btn-outline' : 'y-btn'" :disabled="busy === item.id" @click="toggleResolved(item)">
