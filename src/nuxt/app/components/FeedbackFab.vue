...
async function send () {
  ...
  try {
    ...
    setTimeout(() => { if (sent.value) open.value = false }, 1800)
  } catch (cause) {
...
<template>
  <div v-if="canShow" class="fab-root">
    <button class="fab" type="button" title="Send feedback or report a bug" @click="show">
    ...
    <Teleport to="body">
      <div v-if="open" class="fb-overlay" @click="close">
        <div class="fb-modal" role="dialog" aria-modal="true" aria-labelledby="fb-title" @click.stop>
          <div class="fb-head">
            <div>
              <div id="fb-title" class="fb-title">Send feedback</div>
              <div class="fb-sub">Bugs, ideas, confusing bits — anything goes. It lands straight in Jon's inbox.</div>
            </div>
            <button class="fb-x" type="button" aria-label="Close" :disabled="sending" @click="close">✕</button>
          </div>

          <div v-if="sent" class="fb-thanks" role="status">
            <div class="fb-thanks-big">Thank you!</div>
            <div>Your note is on its way.</div>
          </div>

          <form v-else class="fb-form" @submit.prevent="send">
            <label class="y-field">
              <span>Your name</span>
              <input v-model="name" type="text" maxlength="80" autocomplete="name" required :disabled="sending">
            </label>

            <div class="fb-body-label">Feedback</div>
            <ClientOnly>
              <FeedbackEditor ref="editorRef" v-model="body" :disabled="sending" />
              <template #fallback>
                <textarea v-model="body" class="fb-fallback" rows="7" placeholder="What happened? What did you expect?" />
              </template>
            </ClientOnly>
            ...
            <div v-if="error" class="fb-error" role="alert">{{ error }}</div>

            <div class="fb-actions">
              <button type="button" class="y-btn-secondary" :disabled="sending" @click="close">Cancel</button>
              <button type="submit" class="y-btn" :disabled="!canSend">{{ sending ? 'Sending…' : 'Send feedback' }}</button>
            </div>
          </form>
        </div>
      </div>
