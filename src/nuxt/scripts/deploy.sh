#!/usr/bin/env bash
# Deploy Plannrr to Cloudflare Workers — your own account, your own resources.
...
missing=()
for key in NUXT_CF_ACCOUNT_ID NUXT_CF_D1_DATABASE_ID NUXT_CF_D1_DATABASE_NAME NUXT_CF_KV_ID NUXT_CF_KV_CACHE_ID NUXT_CF_R2_BUCKET; do
  [ -n "${!key:-}" ] || missing+=("$key")
done
...
WORKER_NAME="${NUXT_CF_WORKER_NAME:-plannrr}"
