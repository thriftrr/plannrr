...
export async function fetchLiveDebtSources (token: string, planIds?: string[]): Promise<DebtSource[]> {
  const now = new Date()
  ...
  const { plans } = await ynabApi<{ plans: Array<{ id: string, name: string }> }>(token, '/plans')
  const selected = planIds ? plans.filter(plan => planIds.includes(plan.id)) : plans
  const sources: DebtSource[] = []

  for (const plan of selected) {
    const { accounts } = await ynabApi<{ accounts: ApiAccount[] }>(token, `/plans/${plan.id}/accounts`)
